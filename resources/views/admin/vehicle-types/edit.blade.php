...
<div class="container mx-auto px-4 py-8">
    <div class="flex items-center mb-6">
        ...
        <h1 class="text-3xl font-bold text-drive-teal">Modifier le type de véhicule</h1>
    </div>
        ...
        <form action="{{ route('admin.vehicle-types.update', $vehicleType->id) }}" method="POST" enctype="multipart/form-data">
            ...
            <div class="grid grid-cols-1 gap-6">
                ...
                <div>
                    ...
                    <input type="text" name="name" id="name" value="{{ old('name', $vehicleType->name) }}" 
                ...
                <!-- Nouvelle icône -->
                <div>
                    <label for="icon" class="block text-sm font-medium text-gray-700 mb-1">{{ $vehicleType->icon ? 'Changer l\'icône' : 'Icône' }}</label>
                ...
                <div>
                    ...
                    <textarea name="description" id="description" rows="4" 
                        class="w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-drive-teal focus:border-drive-teal @error('description') border-red-500 @enderror">{{ old('description', $vehicleType->description) }}</textarea>
                    @error('description')
                        <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
                    @enderror
                </div>
