...
<div class="container mx-auto px-4 py-8">
    <div class="flex items-center mb-6">
        ...
        <h1 class="text-3xl font-bold text-drive-teal">Modifier la marque</h1>
    </div>
    ...
    <div class="bg-white rounded-lg shadow-md p-6">
        <form action="{{ route('admin.brands.update', $brand->id) }}" method="POST" enctype="multipart/form-data">
            ...
            <div class="grid grid-cols-1 gap-6">
                ...
                @if($brand->logo)
                    <div>
                        <label class="block text-sm font-medium text-gray-700 mb-1">Logo actuel</label>
                ...
                <!-- Nouveau logo -->
                <div>
                    <label for="logo" class="block text-sm font-medium text-gray-700 mb-1">{{ $brand->logo ? 'Changer le logo' : 'Logo' }}</label>
                ...
                <div>
                    ...
                    <textarea name="description" id="description" rows="4" 
                        class="w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-drive-teal focus:border-drive-teal @error('description') border-red-500 @enderror">{{ old('description', $brand->description) }}</textarea>
                    @error('description')
                        <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
                    @enderror
                </div>
