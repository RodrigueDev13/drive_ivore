...
<div class="container mx-auto px-4 py-8">
    <div class="flex items-center mb-6">
        ...
        <h1 class="text-3xl font-bold text-drive-teal">Modifier l'avis</h1>
    </div>
            ...
            <div class="grid grid-cols-1 gap-6">
                ...
                <div>
                    ...
                    <select name="vehicle_id" id="vehicle_id" 
                        ...
                        @foreach($vehicles as $vehicle)
                            <option value="{{ $vehicle->id }}" {{ old('vehicle_id', $review->vehicle_id) == $vehicle->id ? 'selected' : '' }}>
                ...
                <div>
                    ...
                    <div class="flex items-center">
                        <div class="flex space-x-1">
                            @for($i = 1; $i <= 5; $i++)
                                <label for="rating-{{ $i }}" class="cursor-pointer">
                                    <input type="radio" name="rating" id="rating-{{ $i }}" value="{{ $i }}" class="hidden" {{ old('rating', $review->rating) == $i ? 'checked' : '' }}>
                        ...
                        <span id="rating-text" class="ml-2 text-sm text-gray-500">{{ $review->rating }} étoile{{ $review->rating > 1 ? 's' : '' }}</span>
                    </div>
                    @error('rating')
                        <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
                    @enderror
                ...
                <div>
                    ...
                    <textarea name="comment" id="comment" rows="4" 
                        class="w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-drive-teal focus:border-drive-teal @error('comment') border-red-500 @enderror">{{ old('comment', $review->comment) }}</textarea>
                    @error('comment')
                        <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
                    @enderror
                </div>
                ...
                <div>
                    ...
                    <div>
                        ...
                        @endif
                        <p class="text-sm text-gray-500 mt-1">Pour changer le statut, utilisez le bouton d'approbation sur la page de détails.</p>
                    </div>
                </div>
            </div>
